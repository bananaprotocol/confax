...
Confax.registerCommand('speak', 'moderator', (message, bot) => {
  ...
  const myChannel = message.mentions.channels.array()[0]
  const deleteMessage = message.guild.member(bot.user).hasPermission('MANAGE_MESSAGES')
  ...
  const channel = message.guild.channels.find('name', myChannel.name)
